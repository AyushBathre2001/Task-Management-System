        ...
        <section class="text-gray-600 body-font relative">
          ...
          <div class="container px-5 py-24 mx-auto flex items-center justify-center">
            <div class="lg:w-2/3 md:w-1/2 bg-white rounded-lg p-8 flex flex-col items-center justify-center  w-full mt-10 md:mt-0 relative z-10 shadow-md">
              ...
              <button disabled = {newtask.title && newtask.description ? false: true} onClick={addTask} class="text-white disabled:bg-blue-200 bg-blue-500 border-0 py-2 px-6 focus:outline-none hover:bg-blue-600 rounded text-lg">Create</button>
            </div>
          </div>
        </section>
      </div>
      ...
      <div className="alltasks w-full flex items-center justify-center flex-col">
        ...
        <section class="text-gray-600 body-font">
          <div class="container px-12 py-24 mx-auto flex flex-wrap">
            <div class="flex flex-wrap items-center justify-center min-h-[30vh] -m-4">
              {(allTasks === null || allTasks.length < 1) && <p>Nothing is here.</p>}

              {allTasks &&
                allTasks.map((item, index) => {
                  return <div key={item.id} class=" lg:w-[500px] m-2 md:w-full shadow-lg bg-white rounded-lg">
                    <div class="flex border-2 rounded-lg border-gray-200 border-opacity-50 p-8 sm:flex-row flex-col">
                      ...
                      <div class="flex-grow">
                        ...
                        <div className="w-full flex items-center justify-end p-2 gap-2">
                          <i onClick={() => { setShowModal(true); setDynamicProp({ id: item.id, title: item.title, desc: item.description, status: item.status }) }} class="ri-edit-box-fill text-2xl text-yellow-500 cursor-pointer"></i>
                          <i onClick={() => { deleteTask(item.id,item.status) }} class="ri-delete-bin-5-fill text-2xl text-red-500 cursor-pointer"></i>
                        </div>
